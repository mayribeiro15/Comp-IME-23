...
class CornersProblem(search.SearchProblem):
    ...
    Trying the state space: [(x,y), a, b, c, d]
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            hitsWall = self.walls[nextx][nexty]
            if not hitsWall:
                nexta = a or (nextx,nexty)==(1,1)
                nextb = b or (nextx,nexty)==(1,top)
                nextc = c or (nextx,nexty)==(right, 1)
                nextd = d or (nextx,nexty)==(right, top)
                nextState = ((nextx, nexty),(nexta,nextb,nextc,nextd))
...
def cornersHeuristic(state, problem):
    ...
    for z in [a,b,c,d]:
        ...
        distance = (x-goalx)^2+(y-goaly)^2
        if z==0 and heuristic>distance:
            heuristic=distance
        i=i+1
