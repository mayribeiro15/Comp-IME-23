...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #print(noisyDistance, emissionModel, pacmanPosition, self.beliefs)
        "*** YOUR CODE HERE ***"
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        legalPermutations = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(legalPermutations)
        permutations = itertools.cycle(legalPermutations)
        self.particles = [next(permutations) for _ in range(self.numParticles)]
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                ghostPosition = p[i]
                if noisyDistance == None:
                    p = self.getParticleWithGhostInJail(p, i) #Ghost was eaten, the particle has to be replaced by this scenario
                    belief *= 1 #Probablity of eaten is 100%
                else:
