...
def SemanticalAnalyzer(lexical, rule):   
    ...
    code_generate = open("codigo-gerado.txt","a+")
    ...
    if rule == IDD_RULE:
        ...
        CHR_ = t_attrib(CHR,None,CHR(char_, lexical.getConst(lexical.secondary_Token)))
        StackSem.append(CHR_)
    
    elif rule == STR_RULE:
        STR_ = t_attrib(STR,None,STR(string_, lexical.getConst(lexical.secondary_Token), lexical.secondary_Token))
        StackSem.append(STR_)
    
    elif rule == NUM_RULE:
        NUM_ = t_attrib(NUM,None,NUM(int_, lexical.getConst(lexical.secondary_Token), lexical.secondary_Token))
        ...
        curFunction = f
        code_generate.write("BEGIN_FUNC "+str(f._.nIndex)+" "+str(f._.nParams)+"\n")

    elif rule == DF_RULE:
        EndBlock()
        code_generate.write("END_FUNC"+"\n")
        ...
        if not CheckTypes(t,bool_):
            ...
            code_generate.write("L"+str(MT_._.label)+"\n")
        ...
        if not CheckTypes(t,bool_):
            Error(lexical, ERR_BOOL_TYPE_EXPECTED)
        code_generate.write("L"+str(ME_._.label)+"\n")
        ...
        if not CheckTypes(t,bool_):
            Error(lexical, ERR_BOOL_TYPE_EXPECTED)
        code_generate.write("\tJMP_BW L"+'0'+"\nL"+str(MT_._.label)+"\n")        
        ...
        StackSem.append(F_)
        code_generate.write("\tDE_REF "+str(n)+"\n")
        ...
        code_generate.write("\tDUP\n\tDUP\n\tDE_REF 1"+"\n")
        code_generate.write("\tINC\n\tSTORE REF 1\n\tDE_REF 1"+"\n")
        ...
        StackSem.append(F_)
        code_generate.write("\tLOAD_TRUE"+"\n")

    elif rule == F_FALSE_RULE:
        FALS_ = StackSem.pop()
        F_ = t_attrib(F,None,F(bool_))
        StackSem.append(F_)
        code_generate.write("\tLOAD_FALSE"+"\n")
        ...
        code_generate.write("\tLOAD_CONST "+str(constPool)+"\n")
        constPool+=1
        ...
        code_generate.write("\tADD "+str(p._.nIndex)+"\n")
        ...
        if p.eKind != VAR_ and p.eKind!=PARAM_:
            ...
            code_generate.write("\tLOAD_REF "+str(p._.nIndex)+"\n")
        StackSem.append(LV_)
        ...
        if not LE_._.err:
            if LE_._.n-1 < f._nParams and LE_._.n != 0:
                ...
                Error(lexical, ERR_TOO_MANY_ARG)
        StackSem.append(F_)
        code_generate.write("\tCALL "+str(f._.nIndex)+"\n")

    elif rule == MT_RULE:
        rLabel = newLabel()
        MT_ = t_attrib(MT,None,MT(rLabel))
        StackSem.append(MT_)
        code_generate.write("\tTJMP_FW L"+str(rLabel)+"\n")
        ...
        MW_._.label = rLabel
        StackSem.append(MW_)
        code_generate.write("L"+str(rLabel)+"\n")
        ...
        if t._ == None or t._.nSize == None:
            code_generate.write("\tSTORE_REF 1\n")
        else:
            code_generate.write("\tSTORE_REF "+str(t._.nSize)+"\n")
