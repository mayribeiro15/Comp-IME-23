...
private ResultSet rs;           // retorna a consulta no banco de dados
    ...
    public List<Calibre> listEntidade() throws Exception {
    Calibre cal = null;                               //Declaracao classe cria um variavel e atribui o valor null;
    List<Calibre> lista = new ArrayList<>();         // Cria objeto lista e estancia a lista com o bean da classe
    con = FabricaConexao.getconnection();       //abre conexao    
    sql = "select id as idcalibre, nome as calibre from calibre;";                   //armazena a instrução sql que será inserida no banco
    ...
    while(rs.next()){                           //cria condição enquanto resultado existe executa proximo passo 
        cal = new Calibre();                          //estância o objeto
        cal.setId(rs.getInt("idcalibre"));              //seta no bean o que trouxer do banco 
        cal.setCalibre(rs.getString("calibre"));         //seta no bean o que trouxer do banco 
        lista.add(cal);                          //adiciona tudo na lista 
    }                                           //fecha condição
